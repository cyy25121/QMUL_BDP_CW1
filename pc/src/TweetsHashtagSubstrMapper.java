...
public class TweetsHashtagSubstrMapper extends Mapper<Object, Text, Text, IntWritable> {
	...
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			while(m2.find()){
				String target = m2.group("hashtag").toLowerCase().replaceAll("#", "");
				for(int i = target.length()-1; i >= 0; i--){
					//for(int j = i; j < target.length(); j++){
					String s = target.substring(i, target.length());
					THashtag.set(s);
